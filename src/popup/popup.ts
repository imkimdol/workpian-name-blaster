...
function getElements(): DynamicElementsContainer {
  ...
  const blastButton = document.getElementById(
    "blastButton",
 ...
 */
function changeHTMLElements(
  elements: DynamicElementsContainer,
  isBlastin: boolean,
) {
  if (isBlastin) {
...
async function setup() {
  ...
  try {
    ...
    const isBlastin = await chrome.tabs.sendMessage(activeTabId, {
      action: "isBlastin?",
    });
