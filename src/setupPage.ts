...
class SetupHelper {
    ...
    private async importAlgorithms() { 
        ...
        const algorithmHelper = new helperModule.AlgorithmHelper(this.extensionInfo) as AlgorithmHelper;
        ...
        for (const p of this.extensionInfo.algorithmPaths) {
            ...
            const algorithm: Algorithm = instantiator(this.extensionInfo, algorithmHelper);
    ...
    private scheduleAlgorithms(interval: number) {
        setInterval(() => {if (this.isBlastin) this.runAlgorithms();}, interval);
    }
};
